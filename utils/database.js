...
export function saveMenuItems(menuItems) {
    db.transaction((tx) => {
        ...
        // let addedItems = menuItems.map((item) => [
        //   item.id,
        //   item.title,
        //   item.price,
        //   item.category,
        // ]);
        // tx.executeSql(
        //   `insert into menuitems (uuid, title, price, category) values ${"(?,?,?,?)".repeat(
        //     menuItems.length
        //   )}`,
        //   ...addedItems
        // );
        tx.executeSql(
            `insert into menuitems (uuid, title, price, category, description, image) values ${menuItems
                .map(
                    (item) =>
                        `('${item.id}', '${item.title}', '${item.price}', '${item.category}', "${item.description}", "${item.image}")`
                )
                .join(", ")}`
        );
    });
